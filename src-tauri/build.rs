...
/// and the `npm` scripts run `fetch:pi` as a `prebuild` / `beforeBuildCommand`
...
/// without ever going through npm. When that happens the .app is silently
...
/// clear message pointing the developer at `npm run fetch:pi`. Debug builds
    ...
    if !bin_path.is_file() {
        panic!(
            "\n\n\
             ...
             Fix: run `npm run fetch:pi` from the repo root before building.\n\
             (Or `npm run build`, which already does this for you.)\n\n\
